...
type ColumnType = ColumnDef<PerformanceInterface, unknown>;
...
export function PerformanceListPage(props: PerformanceListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<PerformanceInterface>>(
    () => `/performances?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: PerformanceInterface) => {
    if (hasAccess('performance', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/performances/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'usage_count', header: 'usage_count', accessorKey: 'usage_count' },
    { id: 'average_duration', header: 'average_duration', accessorKey: 'average_duration' },
    { id: 'average_distance', header: 'average_distance', accessorKey: 'average_distance' },
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/performances/view/${record.id}`} passHref legacyBehavior>
                <Button
