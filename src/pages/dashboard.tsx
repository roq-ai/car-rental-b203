...
import { getUsages } from 'apiSdk/usages';
...
function DashboardPage() {
  ...
  const usageStat = useSWR<PaginatedInterface<UsageInterface>>(
    () => `/usages`,
    () => getUsages({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Usages',
      result: usageStat?.data?.totalCount,
      isLoading: usageStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Reservations',
